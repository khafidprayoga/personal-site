...
<template>
  <section id="latest-article" class="mb-10">
    <div>
      <h2 class="text-2xl font-bold capitalize border-b-4 border-gray-200 animate-on-load">Latest Article</h2>
    </div>
    <div id="blog" class="flex flex-col gap-5 my-3">
      <template v-if="articles">
        <div v-for="(article, index) in articles" :key="article.id" class="flex flex-col gap-1 animate-on-load" :class="`animate-delay-${(index + 1) * 100}`" loading="lazy">
